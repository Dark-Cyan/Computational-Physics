...
def next_gen():
    ...
    if pop.finished == pop.families + pop.families * pop.children:
        ...
        water = False
        for i in pop.population[0].familyMembers:
            if i.distance >= 100000:
                water = True
        print("Best Group:", pop.population[0].parent.color, '\t', "Score:", pop.population[0].parent.score, '\t', "Distance:", (pop.population[0].distance/201), '\t', "Water:", water)
