...
def offspring(a, n):
    ...
    for i in range(n):
        color = (max(min(random.gauss(a.color[0], standardDeviation * 10), 255), 0), max(min(random.gauss(a.color[1], standardDeviation * 10), 255), 0), max(min(random.gauss(a.color[2], standardDeviation * 10), 255), 0))
...
def next_gen():
    ...
    if pop.finished == pop.families + pop.families * pop.children:
        ...
        offspring(pop.population[0].parent, int(pop.families*0.50 - 2))
        ...
        ball = pop.Ball(pop.population[1].parent.ivel, pop.population[1].parent.color)
        family = pop.Family(pop.children, ball, 0.05)
        newPopulation.append(family)
        ...
        # print("PATH:", math.degrees(math.atan2(pop.population[0].parent.vel.y, pop.population[0].parent.vel.x)))
        # hole = phys.HOLE - pop.population[0].parent.pos
        # print("HOLE:", math.degrees(math.atan2(hole.y, hole.x)))
