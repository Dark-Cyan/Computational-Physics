...
class Family:
    ...
    def __init__(self, size, lBound, hBound, standardDeviation, familyNum):
        ...
        for i in range(self.size):
            ball=Ball(random.uniform(lBound, hBound), random.uniform(-13,-12), (self.color), familyNum)
                      #random.uniform(0.5,8), (self.color), familyNum)
...
for i in range(0,180 - int(180/families) ,int(180/families)):
    family = Family(populationSize/families,i,i+(180/families), 10, int(i/int(180/families)))
    population.append(family)
family = Family(populationSize/families,-30.29861881,-30.29861881, 1, families - 1)
population.append(family)
